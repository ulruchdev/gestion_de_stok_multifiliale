...
| `DEC-030` → `DEC-034` | Phasage, reliquat, exploitation, volumétrie, points retirés | — | ⏳ |
...
| `DEC-038` | Import de mise en service : catalogue et stock initial | — | ⏳ |

**31 décisions arrêtées**, dont une remplacée (`DEC-035` → `DEC-037`). Restent **6 points ouverts** : `DEC-030` → `DEC-034` et `DEC-038`.
...
### Lot 4 ter — Mise en service

| Réf | Question |
|---|---|
| DEC-038 | **Import de mise en service.** État vérifié : le produit ne comporte **aucun import** — ni catalogue, ni stock initial, ni tiers. Le seul mouvement de masse est `US-080`, un **export** CSV en P2, sprint 11. Une pharmacie de 2 000 références doit donc créer 2 000 articles à la main **puis** saisir 2 000 quantités, avant sa première vente — `DEC-023` refusant toute vente sur stock à zéro. Le blocage porte sur le **catalogue** autant que sur le stock. Faut-il un import CSV de mise en service en V1, et sur quel périmètre (articles seuls, articles + stock, tiers) ? |

### Lot 5 — Phasage et reliquat

| Réf | Question |
|---|---|
| DEC-030 | **Phasage de livraison** : livraison unique, ou jalons V1 / V1.5 avec schéma complet dès V5 |
| DEC-031 | Traitement des ~60 points mécaniques restants (backlog, chiffrage, CI, gouvernance) |
| DEC-032 | Sauvegarde, restauration, rétention, purge — rien de documenté nulle part |
| DEC-033 | Volumétrie cible et archivage de `mouvement_stock` |
| DEC-034 | Éléments retirés sans spécification (point de fidélité de `diagrams/10`, `DEC-009`) : abandon définitif ou instruction ultérieure |
